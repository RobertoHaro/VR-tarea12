...
func _ready():
	...
	# Set teleport button down to false
...
func _physics_process(delta):
	...
	# Obtener la entrada de los sticks
    var joystick_x = -get_joystick_axis(0)  # Invertir el eje X si es necesario
    var joystick_y = get_joystick_axis(1)

    # Calcular la dirección del movimiento
    var movement_direction = Vector2(joystick_x, joystick_y)

    # Aplicar movimiento a las manos
    if movement_direction.length() > CONTROLLER_DEADZONE:
        # Multiplicar por un factor para ajustar la velocidad del movimiento
        var movement_vector = movement_direction.normalized() * MOVEMENT_SPEED * delta
        
        # Actualizar la posición de las manos en función del movimiento
        hand_mesh.translation += Vector3(movement_vector.x, 0, movement_vector.y)

        # Limitar la posición de las manos dentro de ciertos límites si es necesario
        # Por ejemplo, asegúrate de que las manos no se muevan fuera del área de juego
        # Puedes utilizar condicionales y funciones de limitación para esto.

        # Actualizar otras lógicas de movimiento según sea necesario
        # Por ejemplo, actualizar el estado de `directional_movement` si el jugador se está moviendo
	...
	movement_forward = movement_forward * -1
